...
switch option.algorithm
    ...
    case 'sparsenmfNNQP'
        [A,Y,numIter,tElapsed,finalResidual]=sparseNMFNNQP(X,k,option);
    case 'knmf-l1nnqp'
        [AtA,Y,numIter,tElapsed,finalResidual,A]=kernelSparseNMFNNQP(X,k,option);
    case 'seminmfnnls'
        [A,Y,numIter,tElapsed,finalResidual]=seminmfnnls(X,k,option);
    case 'seminmfrule'
        [A,Y,numIter,tElapsed,finalResidual]=seminmfrule(X,k,option);
    case 'convexnmfrule'
        [A,Y,numIter,tElapsed,finalResidual]=convexnmfrule(X,k,option);
        A=X*A;
    case 'sparseseminmfnnls'
        [A,Y,numIter,tElapsed,finalResidual]=sparseseminmfnnls(X,k,option);
    case 'knmf-nnls'
        [AtA,Y,numIter,tElapsed,finalResidual]=kernelseminmfnnls(X,k,option);
        A=AtA;
    case 'knmf-dc'
        [A,Y,numIter,tElapsed,finalResidual]=kernelnmfdecom(X,k,option);
    case 'knmf-ur'
        option.optionnmf
        [AtA,Y,numIter,tElapsed,finalResidual]=kernelseminmfrule(X,k,option);
        A=AtA;
    case 'knmf-cv'
        [~,A,Y,numIter,tElapsed,finalResidual]=kernelconvexnmf(X,k,option);
%         [~,A,Y,numIter,tElapsed,finalResidual]=kernelnmfrule(X,k,option.optionnmf);
    ...
    case 'vsmf'
        [A,Y,numIter,tElapsed,finalResidual]=vsmf(X,k,option);
    otherwise
        eval(['[A,Y,numIter,tElapsed,finalResidual]=',option,'(X,k,option.optionnmf);']);
end
end
