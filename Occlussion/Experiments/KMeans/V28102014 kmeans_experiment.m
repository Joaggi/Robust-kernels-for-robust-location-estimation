...
%         [labels_pred, energy] = knkmeans(K, k);
        labelsPred=kmeans(X,k,'Emptyaction','singleton');
        clear energy;
        contRecur = 0;
...
%                 [labels_pred, energy] = knkmeans(K, k);
            ...
            [labelsPred,vectorCentroids(:,:,i)] = bestMapNMF(labels,labelsPred(1:size(X,1)),vectorCentroids(:,:,i));
            vectorCentroidsContamination(:,:,i) = get_centroids(labelsPred,[X;vectorContamination(1:j,:)])
            [labelsPred,vectorCentroidsContamination(:,:,i)] = bestMapNMF(labels,labelsPred(1:size(X,1)),vectorCentroidsContamination(:,:,i));
    end
    centroid = mean(vectorCentroids,3);
