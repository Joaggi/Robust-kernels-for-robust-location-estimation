...
class Robust(object):
    ...
    def contamination_generator(self,partition = 4, num_outliers_total =100):
        ...
        for i in range (partition):
            ...
            contamination.append(np.random.uniform(low=-7,high=-5,size=num_outliers_part).reshape(num_outliers_part,1))
            contamination.append(np.random.uniform(low=-2,high=-1,size=num_outliers_part).reshape(num_outliers_part,1))
            contamination.append(np.random.uniform(low=2,high=3,size=num_outliers_part).reshape(num_outliers_part,1))
            contamination.append(np.random.uniform(low=4.5,high=7,size=num_outliers_part).reshape(num_outliers_part,1))
...
np.savez('DataGaussian/dataExperiment4', mean = data[[0,2,4]] ,data = data[[1,3,5]][0], contamination = contamination)
